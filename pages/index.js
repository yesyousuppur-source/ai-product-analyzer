...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const acc = accounts.find(a => a.email === saved.email);
      if (acc) setForm({ email: acc.email, password: acc.password, name: acc.name || "" });
      setUser(saved); setUsageInfo(getUsageInfo(saved)); setScreen("dashboard");
    } else {
      // Auto fill last used account
      if (accounts.length > 0) {
        const last = accounts[accounts.length - 1];
        setForm({ email: last.email, password: last.password, name: last.name || "" });
      }
      setScreen("auth");
    }
  }, []);
  ...
  const saveAccount = (email, name, password) => {
    const accounts = store.get("apa_saved_accounts") || [];
    const idx = accounts.findIndex(a => a.email === email);
    const acc = { email, name, password };
  ...
  const handleAuth = () => {
    ...
    if (authMode === "signup") {
      if (users[form.email]) { setError("Email already registered! Please login."); return; }
      ...
      saveAccount(form.email, form.name, form.password);
      ...
      saveAccount(form.email, found.name, form.password);
  ...
  const quickLogin = (account) => {
    ...
    const u = { email:found.email, name:found.name, plan:found.plan||"free" };
    store.set("apa_user", u);
    setForm({ email:account.email, password:account.password, name:account.name||"" });
    setUser(u); setUsageInfo(getUsageInfo(u)); setScreen("dashboard");
  };
  ...
  const css = `
    ...
    * { margin:0;padding:0;box-sizing:border-box; }
    ...
    .neu-card { background:#e8ecf1;border-radius:30px;padding:40px 36px;width:100%;max-width:420px;box-shadow:8px 8px 20px #c8cdd5,-8px -8px 20px #ffffff; }
    .neu-avatar { width:90px;height:90px;background:#e8ecf1;border-radius:50%;margin:0 auto 24px;display:flex;align-items:center;justify-content:center;box-shadow:6px 6px 14px #c8cdd5,-6px -6px 14px #ffffff; }
    .neu-avatar svg { width:44px;height:44px;color:#8b8fa8; }
    .neu-title { font-size:24px;font-weight:800;text-align:center;color:#1a1a2e;margin-bottom:6px; }
    .neu-sub { color:#8b8fa8;font-size:14px;text-align:center;margin-bottom:28px; }

    /* AUTH TABS */
    .auth-tabs-neu { display:flex;background:#e8ecf1;border-radius:14px;padding:4px;margin-bottom:24px;box-shadow:inset 4px 4px 10px #c8cdd5,inset -4px -4px 10px #ffffff; }
    ...
    .auth-tab-neu.active { background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;box-shadow:4px 4px 10px rgba(99,102,241,0.4),-2px -2px 8px rgba(255,255,255,0.3); }

    /* NEU INPUT */
    .neu-inp-wrap { position:relative;margin-bottom:16px; }
    ...
    .neu-inp:focus { box-shadow:inset 5px 5px 12px #c0c5cd,inset -5px -5px 12px #f0f4f8; }
    .neu-inp::placeholder { color:#adb5bd; }
    .neu-inp-err { box-shadow:inset 4px 4px 10px #f0c0c0,inset -4px -4px 10px #ffffff; }
    ...
    .err-text { color:#e53e3e;font-size:12px;margin:-8px 0 12px 4px;font-weight:500; }

    /* BUTTONS */
    .neu-btn { width:100%;background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:14px;padding:15px 0;color:#fff;font-weight:800;font-size:15px;cursor:pointer;font-family:'Inter',sans-serif;box-shadow:4px 4px 12px rgba(99,102,241,0.5),-2px -2px 8px rgba(255,255,255,0.3);transition:all 0.2s;letter-spacing:0.3px;margin-bottom:14px; }
    .neu-btn:hover { transform:translateY(-1px);box-shadow:6px 6px 16px rgba(99,102,241,0.55); }
    .neu-btn:active { transform:translateY(1px);box-shadow:2px 2px 8px rgba(99,102,241,0.4); }
    ...
    .divider-txt { color:#adb5bd;font-size:12px;font-weight:500;white-space:nowrap; }

    /* SAVED ACCOUNTS */
    .saved-wrap { margin-bottom:20px; }
    .saved-lbl { color:#8b8fa8;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px; }
    .saved-acc { display:flex;align-items:center;gap:12px;background:#e8ecf1;border:none;border-radius:16px;padding:12px 14px;margin-bottom:8px;cursor:pointer;width:100%;text-align:left;box-shadow:4px 4px 10px #c8cdd5,-4px -4px 10px #ffffff;transition:all 0.2s; }
    .saved-acc:hover { box-shadow:6px 6px 14px #c0c5cd,-6px -6px 14px #ffffff;transform:translateX(2px); }
    .saved-avt { width:38px;height:38px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:15px;color:#fff;flex-shrink:0;box-shadow:2px 2px 6px rgba(99,102,241,0.4); }
    ...
    .auth-switch { color:#8b8fa8;font-size:13px;text-align:center;margin-top:16px; }
    ...
    .remember label { color:#8b8fa8;font-size:13px;cursor:pointer; }

    /* DASHBOARD */
    body.dash-body { background:#020817; }
    ...
    .upgrade-btn { background:linear-gradient(135deg,#f59e0b,#ef4444);border:none;border-radius:100px;padding:8px 18px;color:#fff;font-weight:800;font-size:13px;cursor:pointer;font-family:'Inter',sans-serif;box-shadow:0 4px 14px rgba(245,158,11,0.35); }
    .avatar { width:36px;height:36px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;color:#fff; }
    ...
    .inp { width:100%;background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:13px 16px;color:#f8fafc;font-size:14px;font-family:'Inter',sans-serif;outline:none;transition:all 0.2s;margin-bottom:0; }
  ...
  if (screen === "loading") return (
    <>
      ...
      <div style={{minHeight:"100vh",background:"#e8ecf1",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
        <div className="spinner" style={{border:"3px solid rgba(99,102,241,0.2)",borderTop:"3px solid #6366f1"}}/>
  ...
  return (
    <>
            ...
            {!showPayment ? (
              <>
                ...
                <div className="prem-highlight">
                  ...
                  ✅ <b>Zero ads</b> — completely ad-free<br/>
                  ✅ <b>30 analyses</b> valid for 7 days<br/>
                  ✅ <b>All 8 platform strategies</b> unlocked
                </div>
                <div className="prem-features">
                  {["📺 Ads on 8+ platforms (Google, Meta, YouTube, TikTok...)","🎬 Step-by-step Video Publishing Guide","🔑 Platform-specific keywords & scripts","💰 Budget recommendations & ROI tips","🚫 No interstitial ads ever"].map(f=>(
              ...
              <>
                ...
                <p className="pay-note">⚠️ Integrate Razorpay SDK for real payments. This demo simulates success.</p>
      ...
      {/* AUTH - NEUMORPHIC DESIGN */}
      {screen==="auth" && (
        <div className="neu-page">
          <div className="neu-card">
            {/* Avatar */}
            <div className="neu-avatar">
              <svg viewBox="0 0 24 24" fill="none" stroke="#8b8fa8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <p className="neu-sub">{authMode==="login"?"Please sign in to continue":"Join YesYouPro for free"}</p>

            {/* Saved Accounts - only on login */}
              ...
              <div className="saved-wrap">
                ...
                {savedAccounts.map(acc=>(
                  <button key={acc.email} className="saved-acc" onClick={()=>quickLogin(acc)}>
                    <div className="saved-avt">{acc.name?.[0]?.toUpperCase()||"U"}</div>
                    <div className="saved-info">
                      <div className="saved-name">{acc.name}</div>
                      <div className="saved-email">{acc.email}</div>
                      <div className="saved-pass">{"●".repeat(Math.min(acc.password?.length||6,8))}</div>
                    </div>
                ...
                <div className="divider">
                  ...
                  <div className="divider-txt">or sign in manually</div>
                  <div className="divider-line"/>
                </div>
              </div>
            )}
            ...
            {/* Google Button */}
            <button className="neu-google-btn" onClick={()=>showToast("Add Firebase Auth to enable Google login")}>
            ...
            <div className="divider">
              <div className="divider-line"/>
              <div className="divider-txt">OR CONTINUE WITH EMAIL</div>
              <div className="divider-line"/>
            </div>
            ...
            {authMode==="signup" && (
              <div className="neu-inp-wrap">
                ...
                <input className={"neu-inp"+(error&&!form.name?" neu-inp-err":"")} placeholder="Full Name" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/>
              </div>
            )}

            <div className="neu-inp-wrap">
              <span className="neu-inp-icon">✉️</span>
              <input className={"neu-inp"+(error&&!form.email?" neu-inp-err":"")} placeholder="Email address" type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/>
            </div>

            <div className="neu-inp-wrap">
              <span className="neu-inp-icon">🔒</span>
              <input className={"neu-inp"+(error&&!form.password?" neu-inp-err":"")} placeholder="Password" type={showPass?"text":"password"} value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/>
            ...
            {authMode==="login" && (
              <div className="remember">
                ...
                <span style={{marginLeft:"auto",color:"#6366f1",fontSize:13,cursor:"pointer",fontWeight:600}}>Forgot password?</span>
              </div>
            )}
              ...
              <div className="fade-in">
                ...
                <div className="platforms-section">
                  ...
                  <div className="platforms-grid">
                    {PLATFORMS.map(p=>(
                      <div key={p.id} className={"platform-btn"+(selectedPlatform===p.id?" active":"")+((user?.plan||"free")==="free"?" ":"")} onClick={()=>fetchPlatformData(p.id)} style={{borderColor:selectedPlatform===p.id?p.color:undefined}}>
                      ...
                      {platformLoading ? (
                        ...
                        return (
                          <>
                            ...
                            {d.script&&<div className="pd-block"><div className="pd-title">📝 Script / Caption</div><div className="pd-text" style={{background:"rgba(99,102,241,0.06)",padding:14,borderRadius:10,border:"1px solid rgba(99,102,241,0.15)",lineHeight:1.75}}>{d.script}</div></div>}
                            {d.video_steps&&<div className="pd-block"><div className="pd-title">🎬 Video Publishing Steps</div><div className="pd-steps">{d.video_steps.split("\n").filter(Boolean).map((s,i)=><div key={i} className="pd-step"><span className="pd-step-num">{i+1}</span><span>{s.replace(/^\d+[\.\)]\s*/,"")}</span></div>)}</div></div>}
                            ...
                            {d.budget&&<div className="pd-block"><div className="pd-title">💰 Budget & ROI</div><div className="pd-text">{d.budget}</div></div>}
                          </>
                        );
                      })():null}
                    </div>
                  ...
                  {(user?.plan||"free")==="free" && (
                    <div className="lock-box">
                      ...
                      <div className="lock-sub">Unlock complete ad strategies + video publishing guides for all 8 platforms</div>
